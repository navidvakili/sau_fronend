          ...
          {inspectorTab === 'content' && (
            <div className="space-y-4">
              ...
              {selectedWidget.type === 'child-pages' && (
                <>
                  <div className="space-y-1.5">
                    <label className="text-[11px] font-bold text-slate-700 dark:text-slate-300">حالت نمایش</label>
                    <div className="flex items-center gap-2">
                      <button
                        onClick={() => updateCustomProps({ displayMode: 'list' })}
                        className={`px-3 py-2 rounded-xl text-[11px] font-bold border transition-all cursor-pointer ${
                          customProps.displayMode !== 'grid'
                      ...
                      <button
                        onClick={() => updateCustomProps({ displayMode: 'grid' })}
                        className={`px-3 py-2 rounded-xl text-[11px] font-bold border transition-all cursor-pointer ${
                          customProps.displayMode === 'grid'
                            ? 'bg-teal-500 text-white border-teal-500 shadow-sm'
                            : 'bg-slate-100 dark:bg-slate-950 text-slate-500 border-gray-200 dark:border-slate-800'
                  ...
                  <label className="flex items-center gap-2 text-[11px] font-bold text-slate-700 dark:text-slate-300 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={customProps.showDates !== false}
                      onChange={(e) => updateCustomProps({ showDates: e.target.checked })}
                      className="accent-teal-500 w-3.5 h-3.5 cursor-pointer"
                    />
                    نمایش تاریخ انتشار
                  </label>
                  <p className="text-[10px] text-slate-400 dark:text-slate-500 leading-relaxed">
                    این ویجت به‌صورت خودکار زیرصفحه‌های صفحهٔ فعلی را فهرست می‌کند؛ ابتدا زیرصفحه‌ها را از «تنظیمات صفحه» با انتخاب صفحهٔ والد بسازید.
                  </p>
                </>
              )}
