      ...
      <header className="h-16 px-4 bg-white dark:bg-slate-900 border-b border-gray-200 dark:border-slate-800 flex items-center justify-between shrink-0 shadow-xs rounded-2xl">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <h1 className="text-sm font-black text-slate-900 dark:text-white truncate">مدیریت اطلاعیه‌ها</h1>
            <div className="flex items-center gap-1.5 text-[10px] text-slate-400 flex-wrap">
              ...
              <span className="px-1.5 py-0.5 rounded-md bg-rose-500/10 text-rose-600 dark:text-rose-400 font-bold border border-rose-500/20">
                {importantCount} فوری
              </span>
              {activeLanguage && (
                  ...
                  <div className="p-5 flex-1 flex flex-col justify-between space-y-4">
                    <div className="space-y-2">
                      <div className="flex items-center flex-wrap gap-2">
                        ...
                        {item.type === 'important' && (
                          <div className="px-3 py-1 rounded-full bg-rose-600 text-white font-black text-[10px] shadow-md flex items-center gap-1">
                            <AlertCircle className="w-3 h-3" />
                            <span>فوری</span>
                          </div>
                        )}

                        <div className={`px-3 py-1 rounded-xl font-bold text-[11px] ${item.category_color ? item.category_color : 'bg-teal-600/90 text-white'}`}>
                          {item.category_name || item.group || 'عمومی'}
                        </div>

                        {item.status !== 'published' && (
                          <div className="px-2.5 py-1 rounded-full bg-amber-600 text-white font-bold text-[10px]">
                            پیش‌نویس
                          </div>
                        )}
                      </div>
