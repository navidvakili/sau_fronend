...
4. Map each identified element to the closest widget type using `reference/widget-catalog.md`.

   **Two different kinds of gap need two different responses — don't treat them the same:**
   - **A widget type exists but is missing one setting/field for this instance** (e.g. a `customProps` value not documented in the catalog) → handled in Step 3 below: you may add the field yourself with the backend's opaque-JSON tolerance, but tell the user what you added and get confirmation before finalizing.
   - **No widget type at all can represent the interaction** — tabs/accordSwitcher-driven panels, an interactive multi-marker map (pin-switching), file-upload-and-process blocks, any real client-side behavior beyond what a static render can do → this is a **missing capability**, not a missing setting, and building it for real means writing new code in `builderTypes.ts`/`WidgetRenderer.tsx`/`InspectorPanel.tsx` (a Page Builder feature, not a one-page fix). **Stop and ask the user** before silently downgrading to a static `custom-html` substitute: tell them what the source page does, that no widget supports it, and ask whether they want (a) a real new widget type built for it (scope that as its own task), (b) a static/simplified `custom-html` approximation for now, or (c) to skip that element. Only proceed with a silent `custom-html` substitution if the user has already told you in this conversation that approximations are acceptable for this specific clone.
...
**If the source page has a real visual/structural detail that no existing widget setting can represent**, you may add extra fields into that widget's `settings.customProps` (or lesser-known `settings.style` fields already defined in `WidgetStyle`) to capture it — the backend stores `schema` as an opaque JSON blob, so extra fields round-trip safely with zero backend changes. But always tell the user exactly what you added and why, and get their confirmation before finalizing the page — never invent a field silently and never touch `builderTypes.ts`/`WidgetRenderer.tsx` itself without being asked to.

**"Showcase card" sections (a card floating over/between two other sections, e.g. a president/director spotlight, a featured-callout band) need two nested levels, not one flat section:**
- The **outer** `SectionInstance` — the one carrying the overlap (`marginTop: -N` to ride up over the section above) — must be `layout: 'full-width'`, with a single `width: 12` column whose `blocks` holds exactly one `{ kind: 'section', section: {...} }` entry (no visible background/shadow/radius of its own; `paddingTop`/`paddingBottom` can be `0`).
- The **inner nested section** is where the actual card look lives: `layout: 'boxed'` (renders `max-w-[1200px] mx-auto`, which is what visually centers/narrows the card) plus `backgroundColor`, `boxShadow`, `borderRadius`, and the real `paddingTop`/`paddingBottom`, and it holds the real content columns.
- Reason: `layout: 'boxed'` directly on a single section still spans the section's own boundary; the full-width-outer/boxed-inner split is what actually produces a card that reads as centered content sitting inside a full-bleed band. This is the same pattern already used for grid rows (see `sec-leadership-cards` in this skill's own example output) — apply it consistently to any "spotlight card" section, not just grids.

**Never use `callout` for a small icon+text pill/badge/chip** (e.g. a short tag line above a hero heading, a small labeled section marker). `callout` is a full alert-box component (icon-in-a-square + bold title line + separate body paragraph) and is always full-width in its column by default — a `boxed` *section* does not shrink it. For an actual compact pill/chip, use `reference/widget-catalog.md`'s "Icon + short-text badge/chip pattern" instead: a `heading`/`text` widget with an inline `[icon:name]` token in `content`, plus `settings.style.backgroundColor` + `backgroundOpacity` (not an 8-digit alpha hex) for the tint, `borderWidth`/`borderColor` for the outline, and `widthMode: 'center'` + `maxWidth` to keep it pill-sized. Also see that same file's fixed valid-`iconName`-list — `iconName` and `[icon:name]` tokens both draw from one small fixed map, not general lucide-react names, and a wrong guess silently renders the wrong icon instead of erroring.

**There is no widget for an arbitrary small "icon + label + value" row** (e.g. "ایمیل مستقیم: x@y.com" / "تلفن دفتر: ۰۳۵-..." shown as two separate compact rows). Don't approximate this with a single `text` widget concatenating both — it collapses the visual structure. The closest real fit is `icon-box` with `customProps.layout: 'row'` — one `icon-box` per row, with `widget.title` as the label (small, muted) and `widget.content` as the value (bold), `customProps.iconName`/`iconColor`/`iconBgColor` for the icon, and `settings.style.paddingTop/Bottom/Left/Right` + `backgroundColor` + `borderRadius` tightened down to mimic a compact pill-style row (icon-box's default look is a larger `p-6` card, so override padding explicitly to keep it row-sized). Do **not** reach for `contact-info` for this — that widget always renders a fixed 4-row card (`تلفن`/`ایمیل`/`نشانی`/`ساعات کاری`, in that order, all four every time) and cannot be limited to a custom subset or custom labels.
