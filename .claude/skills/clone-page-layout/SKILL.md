...
**The `callout` widget is always full-width within its column by default** — it has no dedicated "boxed/badge" mode of its own, and a `boxed` *section* does not shrink it (section-level boxed only narrows the section's outer bounds; each widget inside still fills 100% of its column unless the widget's own style says otherwise). To use `callout` as a small centered pill/badge (not a full-width alert bar), you must explicitly set `settings.style.widthMode: 'center'` and a `settings.style.maxWidth` (px) on that widget instance — these are generic `WidgetStyle` fields honored by every widget type, not something specific to callout. Forgetting this is a common mistake: the badge will render as a full-width amber alert box instead of a compact pill.
